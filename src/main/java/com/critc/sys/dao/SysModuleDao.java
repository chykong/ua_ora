...
public class SysModuleDao extends BaseDao {
    ...
    public int add(SysModule sysModule) {
        String sql = "insert into t_sys_module(id,name,code,parent_id,url,target,iconImg,display_order) values(seq_t_sys_module.nextval,:name,:code,:parent_id,:url,:target,:iconImg,:display_order)";
    ...
    public SysModule get(int id) {
        String sql = "select t.*,(select name from t_sys_module where id=t.parent_id) parent_name from t_sys_module t where id=?";
