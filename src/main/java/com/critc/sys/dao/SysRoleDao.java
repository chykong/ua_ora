...
public class SysRoleDao extends BaseDao {
    ...
    public int add(SysRole sysRole) {
        String sql = "insert into t_sys_role(id,name,description,create_date,is_delete,create_person)" + " values(seq_t_sys_role.nextval,:name,:description,sysdate,:is_delete,:create_person)";
        KeyHolder keyHolder = new GeneratedKeyHolder();
        int rc = getNamedParameterJdbcTemplate().update(sql, new BeanPropertySqlParameterSource(sysRole), keyHolder, new String[] { "id" });
        if (rc > 0) {
